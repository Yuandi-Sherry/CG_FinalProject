...
void ParticleGenerator::Draw()
{
	...
	{
		if (particle.Life > 0.0f && particle.Position.y > 0)
		{
			GLfloat vertices[] = {
				particle.Position.x, particle.Position.y - 1.0f, particle.Position.z, 1.0f, 1.0f, 1.0f,
...
GLuint ParticleGenerator::firstUnusedParticle()
{
	...
	for (GLuint i = lastUsedParticle; i < this->amount; ++i) {
		if (this->particles[i].Life <= 0.0f || this->particles[i].Position.y < 0) {
			lastUsedParticle = i;
			return i;
		}
	}
