...
void ParticleGenerator::init() {
	...
	mUpdateShader = new Shader("Update.vs", "Update.fs", "Update.gs", varyings, 5);
	//设置TransformFeedback缓存能够记录的顶点的数据类型

	mRenderShader = new Shader("Render.vs", "Render.fs");
	//设置随机纹理
	initRandomTexture(512);
	mParticleTexture = utils::loadTexture((GLchar *)"snowstorm.bmp");
	mRenderShader->use();
	mRenderShader->setInt("snowflower", 0);
	glUseProgram(0);
	initSnow();
}
bool ParticleGenerator::initSnow()
{
...
void ParticleGenerator::display(float frametimeMills)
{
...
void ParticleGenerator::updateParticles(float frametimeMills)
{
	mUpdateShader->use();
...
void ParticleGenerator::renderParticles(glm::mat4& worldMatrix,
...
void ParticleGenerator::initRandomTexture(unsigned int size)
{
	srand(time(NULL));
...
void ParticleGenerator::genInitLocation(SnowParticle particles[], int nums)
{
	srand(time(NULL));
